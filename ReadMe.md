...
- Basic Time Awareness - Allows the AI to know how long it has been since your last reply. You can set up an expected time between answers up to 168 hours. Make sure you only use integers that don't include any decimal numbers. (Includes an easter egg inside the code.)
- Vtube Studio Addon (Parameter control through Vtube Studio's API's) - Animations are still being created, however as it's functional, I'll be including it in the update. (Animations are hard as they involve Math and a lot of time to get them right hahaha) - If you want, feel free to create your own animations by creating your movement formulas! - Additionally it's now possible to reset the setup of plugin for Vtube Studio if it breaks or you need to reset it (Haven't managed to break it yet however it will be here if needed).
- Addons Menu - Where you can easily enable Addon's or disable them. Currently only has an empty menu saying none exist. - Note that some of the Addons will require rebooting for them to work and some are in development.
- Addon Settings Menu - Where you can tweak settings for the Addons.
- Discord Compatibility - As a joke, I've tried to make the AI compatible with Discord. A day later I found a way that worked and, well... it will be kept! You can choose if it lets you know when it's online or not through a message (You can disable this in the Addon Settings). Afterwards, it's able to read the user's input, determine who they are and then answer them, with or without TTS (it will be using discord's TTS rather than the voice built-in and can be disabled through the Addon Settings). Please note that the AI's answer capabilities are based on your model and that prompt engineering is important as well for the better answers. Additionally, the current implementation only allows the usage of the AI for one server's channel only. - I'll eventually make a guide how to set it up, however please feel free to follow the instructions on how to setup the Bot using this video (https://www.youtube.com/watch?v=2k9x0s3awss&ab_channel=pixegami). Do note that the only needed permissions will be the Manage Messages, Read Message History, Send Messages and Send TTS Messages.
...
-  Idle User Awareness - If the AI is too long opened without a reply, it will send a follow up question to try and engage the user.
